...
- [x] 2.2 `github-adapter.test.ts` - PR CRUD, merge, status (mock Octokit)
- [x] 2.3 `openspec-adapter.test.ts` - scaffold, read, archive operations
- [x] 2.4 `environment-adapter.test.ts` - detect env, bootstrap strategies
...
- [x] 3.3 `submit.test.ts` - commit flow, PR creation, archiving
- [x] 3.4 `merge.test.ts` - PR merge, branch cleanup, worktree removal

## 4. Config Tests
- [x] 4.1 `config.test.ts` - loading, validation, defaults, env overrides

## 5. MCP Tool Tests
- [x] 5.1 Tool schema validation tests
- [ ] 5.2 Tool handler integration tests (mocked core)

## 6. CI Integration
- [x] 6.1 Verify tests pass in GitHub Actions
- [x] 6.2 Remove `--passWithNoTests` flag once tests exist
